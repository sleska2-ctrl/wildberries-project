...
Google Sheets больше не используются в основном рабочем процессе из-за низкой производительности на больших объемах данных. Старый код интеграции и отдельные служебные скрипты сохранены только как legacy-инструменты, но актуальная архитектура опирается на SQLite и web UI.
...
- `google-api-python-client` и `google-auth` остались только для legacy-сценариев Google Sheets.
...
- `src/wb_gsheets/google_sheets.py` - legacy-клиент Google Sheets; в основном процессе не используется как целевое хранилище.
...
- `scripts/*.py` - вспомогательные скрипты, часть из них относится к старым Google Sheets-дашбордам.
...
Legacy-переменные для Google Sheets:
...
Эти переменные не нужны для штатной работы SQLite/web UI. Не храните реальные JSON-ключи Google в проекте; каталог `secrets/` игнорируется git и предназначен только для локальных временных секретов.
...
- legacy fallback: если локально пусто и вручную настроен Google Sheets, лист `GOOGLE_COGS_SHEET`;
...
5. Если в SQLite нет SKU и вручную заданы legacy-настройки Google Sheets, справочник может быть прочитан из Google Sheets; штатно этот путь не используется.
...
Google Sheets больше не используются в основном рабочем процессе. От них отказались из-за низкой производительности при росте объема сырых и аналитических данных. Актуальное хранилище - SQLite, актуальный интерфейс просмотра - web UI.

В коде оставлен legacy fallback: если таблица SKU отсутствует в SQLite и вручную настроены `GOOGLE_SERVICE_ACCOUNT_FILE` и `GOOGLE_SPREADSHEET_ID`, основной сценарий может прочитать справочник SKU из Google Sheets. Это не основной путь и не требует хранения Google-ключа в репозитории.
...
Эти скрипты являются legacy/служебными: в них встречаются жестко заданные даты и формулы, завязанные на русскую локаль Google Sheets. Для текущей эксплуатации их лучше не использовать без отдельной проверки.
