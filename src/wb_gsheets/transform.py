...
def filter_orders_rows(
    ...
    for row in rows:
        ...
        if nm_ids:
            ...
            if supplier_article not in supplier_articles:
                continue
...
def build_buyout_order_day_rows(
    ...
    ads_rows: list[dict[str, Any]],
    nm_mapping: dict[str, str] | None = None,
    funnel_data: list[dict[str, Any]] | None = None,
) -> list[list[object]]:
    ...
    for row in orders_rows:
        # raw_orders здесь нужен только для связи nmId с артикулом.
        # Суммы и количество заказов для отчета берем только из funnel_analytics.
        ...
        if nm_id and article:
            nm_to_article[nm_id] = article
        if not order_date or not nm_id:
            continue
    ...
    for row in sales_rows:
        ...
        for_pay = as_decimal(row.get("forPay"))
        event_bucket = grouped[(event_date or order_date, nm_id)]
        ordered_bucket = grouped[(order_date, nm_id)]
        if article and not event_bucket["article"]:
            event_bucket["article"] = article
        if article and not ordered_bucket["article"]:
            ordered_bucket["article"] = article
        if doc_type == "Продажа":
    ...
    for item in (funnel_data or []):
        ...
        vendor_code = str(product.get("vendorCode", "")).strip()
        if not nm_id_f:
            continue
        if vendor_code:
            nm_to_article.setdefault(nm_id_f, vendor_code)
        for history_item in item.get("history", []) or []:
            date_f = str(history_item.get("date", "")).strip()
            if not date_f:
                continue
            ...
            if vendor_code and not bucket_f["article"]:
                bucket_f["article"] = vendor_code

    result = [headers]
    for (order_date, nm_id), bucket in sorted(grouped.items(), key=lambda item: (item[0][0], item[0][1]), reverse=True):
        article = str(bucket["article"] or nm_to_article.get(nm_id, "") or nm_id)
